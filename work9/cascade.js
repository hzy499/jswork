...
collegesSelect.onchange = function(){
    //清空majorSelect下原有的选项<option>
    majorSelect.options.length = 0;
    createOption(majorSelect,majorSelectArr[collegesSelect.value]);
    if ( collegesSelect.value >= 0){
    //自动添加majorSelect下拉列表
        majorSelect.onchange();
    }else{
    //清空classes下原有的<option>
...
majorSelect.onchange = function(){
    //清空classes下所有原有的<option>
    classSelect.options.length = 0;
    createOption(classSelect,classSelectArr[collegesSelect.value][majorSelect.value]);
}
